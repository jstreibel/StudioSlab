...
BenchmarkHistogram::BenchmarkHistogram(BigInt firstClass, BigInt lastClass, BigInt nClasses)
...
void BenchmarkHistogram::storeMeasure(PosInt nSteps) {
    timer.stop();

    BigInt V = timer.elapsed().wall/(1000*nSteps); // ns -> \mu s

    if(V<C0 || V>VLast) return;
    int i = int(floor((V-C0)*invI));
    histogram[i]++;
    count++;
}
