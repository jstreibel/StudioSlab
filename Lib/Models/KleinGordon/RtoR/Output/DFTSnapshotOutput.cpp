...
namespace Slab::Math::RtoR {
    ...
    DiscreteFunction_CPU DFTSnapshotOutput::filterData(const OutputPacket &packet) {
        DiscreteFunction_CPU dft((int)NDFTModes, 0.0, kMaxDFT);

        IN phi = packet.getEqStateData<EquationState>()->getPhi();
        fix pts = DFT::Compute(phi).getMagnitudes()->getPoints();
