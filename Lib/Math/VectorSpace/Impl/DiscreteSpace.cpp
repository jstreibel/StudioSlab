...
namespace Slab::Math {
    ...
    DiscreteSpaceCPU *DiscreteSpace::hostCopy(UInt maxResolution) const {
        ...
        if (newDim.getNDim() == 1)
            for (UInt i = 0; i < dim.getN(0); i++)
                reducedX[UInt(r *
                              i)] = X[i];     // TODO lento bagaray, ridiculamente simples deixar mais rapido vamos combinar
    ...
    const RealArray &DiscreteSpace::getHostData(bool sync) const {
        if (sync) syncHost();
        return data;
    }

    RealArray &DiscreteSpace::getHostData(bool sync) {
        if (sync) syncHost();
        return data;
    }
