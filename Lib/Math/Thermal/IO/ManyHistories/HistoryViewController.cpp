...
namespace ThermoOutput {
    ...
    // Dynamic
    const auto dynamic = MetropolisAlgorithm::Kawasaki;
            ...
            : window(sf::VideoMode(WinW, WinH), "SFML window", style), T(T), MCSteps(MCSteps), tau_eq(tau_eq), L(L), SQRT_SIMS(sqrtNSims)
    {
        ...
        for(int i=0; i<SQRT_SIMS; ++i) {
            for(int j=0; j<SQRT_SIMS; ++j) {
                ...
                auto ic = (!_SINGLE_IC) ? (j >= SQRT_SIMS / 2 ? MetropolisAlgorithm::Paramagnetic  : MetropolisAlgorithm::Ferromagnetic)
...

        {
            if(SHOW_DDT) {
                ...
                subWindow.left += colWidth + _border;
                subWindow.height *= 2;
                subWindow.height += 2 * _border;
            } else {
                subWindow.top += rowHeight + _border;
            }
    ...
    void HistoryViewController::_stepSims(int &MCStep) {
        ...
        for (auto sim : simulations) {
            ...
            if(calc->ic == MetropolisAlgorithm::Ferromagnetic || _SINGLE_IC)
    ...
    void HistoryViewController::__updateIsingGraph(const XYNetwork &S, sf::Texture &networkTexture) {
        u_char neg = .2*255;
        u_char pos = .8*255;
        ...
        for(int i=0; i<S.L; ++i)
            for(int j=0; j<S.L; ++j) {
                auto color = S.theta(i, j) == 1 ? positiveColor : negativeColor;
    ...
    void HistoryViewController::_dumpData() {
        ...
        if(1)
        {
            ...
            auto nameAppend = std::string(dynamic==MetropolisAlgorithm::Kawasaki?"-kawasaki":"-metropolis") + "--space-corr.dat";
