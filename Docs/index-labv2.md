...
- In `WasmIsingWorkspaceSandbox`, `Observable History` belongs to the `Simulation` workspace, is the default selected tab in the bottom dock stack, and shares that taller stack with `Run Inspector` and `Event Console`; `Hysteresis Trace` docks immediately to the right of `Spin Lattice` with an equal-width split, `Phase Snapshot` stays on the far-right rail, the `Observables` workspace keeps `Sampling Controls` plus `Sweep Statistics`, `History points` in `Ising Controls` drives both the observable plots and the `h x m` loop trace, and the preset actions are also available in `Ising Controls` below the secondary separator.
