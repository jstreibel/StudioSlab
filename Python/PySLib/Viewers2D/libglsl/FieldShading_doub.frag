...
void main()
{
    dvec2 fieldInfo = texelFetch(field, v_texcoord);
    float Phi = phiScale * float(fieldInfo.r);
    float dPhidt = dPhidtScale * float(fieldInfo.g);
    ...
    if(lightOn == 1)
    {
        ...
        if(viewMode == 0){
            ...
            fN = float(texelFetch(field, v_texcoord+dy).r);
            fS = float(texelFetch(field, v_texcoord-dy).r);
            fE = float(texelFetch(field, v_texcoord+dx).r);
            fW = float(texelFetch(field, v_texcoord-dx).r);
        } else if(viewMode == 1) {
            scale = dPhidtScale;
