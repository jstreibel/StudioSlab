...
namespace ThermoOutput {
    ...
    // Dynamic
    const auto dynamic = IsingMonteCarloCalculator::Metropolis;
            ...
            MCSteps(tau_eq+total_measures*3*tau_corr), tau_eq(tau_eq), tau_corr(tau_corr), total_measures(total_measures), SQRT_SIMS(sqrtNSims)
    {
        ...
        for(int i=0; i<SQRT_SIMS; ++i) {
            for(int j=0; j<SQRT_SIMS; ++j) {
                ...
                auto calc = new IsingMonteCarloCalculator(L, T, .0, this, IsingMonteCarloCalculator::Ferromagnetic, dynamic, IsingMonteCarloCalculator::Random);
    ...
    auto QuenchViewController::_runProgram() -> bool {
        ...
        const ThermoUtils::Real ΔT = 1.0 / total_measures;
    ...
    void QuenchViewController::__updateIsingGraph(const IsingNetwork &S, sf::Texture &networkTexture) {
        u_char neg = .2*255;
        u_char pos = .8*255;
        ...
        for(int i=0; i<S.L; ++i)
            for(int j=0; j<S.L; ++j) {
                sf::Color color;
                if(!SHOW_ENERGY)
                    color = S.s(i,j)==1?positiveColor:negativeColor;
                else
    ...
    void QuenchViewController::_dumpData() {
...

        {
            ...
            ThermoUtils::Real e, e2, m, m2, m4, T;
