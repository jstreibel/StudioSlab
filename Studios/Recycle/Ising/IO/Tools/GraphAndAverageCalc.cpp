...
GraphAndAverageCalc::GraphAndAverageCalc(sf::IntRect subWindow,
    ...
    : Graph(subWindow, rMin, rMax, xAxisLabel, yAxisLabel), transientTime(transient)
{
    this->avgAbs = avgAbs;
...
void GraphAndAverageCalc::addPoint(float x, float y, sf::Color color) {

    Graph::addPoint(x, y, color);
    ...
    if(currentTime-transientTime > lastManip && computeAverages)
    {
        ...
        auto colorFaded = color; colorFaded.a = 0.1*255;
...
void GraphAndAverageCalc::draw(sf::RenderTarget &target, sf::RenderStates states) const {
    ...
    target.draw(&movingTransientVerticalLine[0], movingTransientVerticalLine.getVertexCount(), movingTransientVerticalLine.getPrimitiveType(), states);
    target.draw(&lastManipTimeVerticalLine[0], lastManipTimeVerticalLine.getVertexCount(), lastManipTimeVerticalLine.getPrimitiveType(), states);

    target.draw(&postTransientDataAvgPlusSigma[0], postTransientDataAvgPlusSigma.getVertexCount(), sf::Points, states);
    target.draw(&postTransientDataAvg[0], postTransientDataAvg.getVertexCount(), sf::Points, states);
    target.draw(&postTransientDataAvgMinusSigma[0], postTransientDataAvgMinusSigma.getVertexCount(), sf::Points, states);
...
AveragesHistory GraphAndAverageCalc::getAveragesHistory() {
    ...
    for(int i=0; i<postTransientDataAvg.getVertexCount(); i++){
        ...
        data[i] = {{avg.x, avg.y}, {avgPlusSigma.y, avgMinusSigma.y}};
    }
